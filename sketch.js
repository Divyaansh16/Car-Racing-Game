...
function setup(){
    ...
    inp = createInput()
    inp.position(window.innerWidth/2-140,200)
    ...
    title.position(window.innerWidth/2-100,50)
    ...
    car1 = createSprite(640,200,30,30)
    car2 = createSprite(860,200,30,30)
...
function draw(){
    ...
    if(gs === 1){
       ...
       for(var i in playerdata){
        ...
        if(index === replace-1){
            fill('purple')
            ellipse(cars[replace-1].x,cars[replace-1].y,50,50)
...
function coins(){
    if(frameCount%80===0){
        var coin=createSprite(Math.round(random(100,900)),-60,25,25)
        coin.velocityY=3
    }
}
function fuels(){
